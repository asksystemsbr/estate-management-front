  ...
  export default {
    ...
    setup(props,{ emit }) {
      ...
      try {
        ...
        const response = await axios.get(`/api/Cliente/${props.id}`);
        console.log('Dados do Cliente Recebidos:', response.data);
        client.value = response.data;

        await nextTick(); // Aguarda a atualização do DOM
        client.value.id_situacao_cliente = response.data.iD_SITUACAO_CLIENTE;
        client.value.cpf_cnpj = response.data.cpF_CNPJ;
    ...
    const updateClient = async () => {
      try {
        await axios.put(`/api/Cliente/${client.value.id}`, client.value);
        clearClient();
        emit('update');
      } catch (error) {
        emit('error', error); 
