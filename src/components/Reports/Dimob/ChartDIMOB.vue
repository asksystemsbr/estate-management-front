    ...
    <v-row  align="center" justify="center">        
      <v-col cols="12" md="12">
        <v-select
            ...
            item-value="id"  
            @update:model-value="handleLocadorSelectClick"
        ></v-select>
      </v-col>
    </v-row>      
    <v-row align="center" justify="center">
...
export default {
  ...
  computed: {
    isFormValid() {
      return this.selectedLocadorId && this.selectedYear !== null && this.selectedImovelId;
    },
    // filteredItems() {
  ...
  methods: {
    ...
    async fetchImovel  ()  {
      try {
        const params = {
        filterId: this.selectedLocadorId,
        StartDate:  null,
         EndDate:  null,
         filterCod: null
      };
      
      const response = await axios.get(`/api/Imovels/getimoveisblocadorview`,{params});
        this.imoveisDisponiveis = response.data.map(imovel => ({
          id: imovel.id,
          codigoImovel: imovel.codigoImovel,
        }));
        console.log(this.imoveisDisponiveis.value);
      } catch (error) {
        this.handleGlobalError(error, 'Erro ao buscar registro');
      }
    },
    async fetchData() {
      try {
        ...
        const response = await axios.get(`/api/Dimob/getItemsView?${params}`);
    ...
    async handleLocadorSelectClick(){
      await this.fetchImovel();
    },
    ...
    try {
        ...
        worksheet.getCell('A1').value = `Planilha de Base de Faturamento - ${this.selectedYear}`;
        ...
        this.report.locatarios.forEach(locatario => {
            ...
            const locatarioEnderecoRow = worksheet.addRow([`Endereço do imóvel: ${this.report.locadorEnderecoImovel} - Município: ${this.report.locadorMunicipioImovel} - UF: ${this.report.locadorUFImovel} - CEP ${this.report.locadorCEPImovel}`]);  
            locatarioEnderecoRow.eachCell((cell) => {
                cell.style = dataStyleLeft;
            });
            worksheet.mergeCells(`A${worksheet.lastRow.number}:N${worksheet.lastRow.number}`);
   ...
   // Ajusta a largura das colunas restantes
      worksheet.columns.forEach((column, index) => {
        ...
        link.download = `DIMOB_${this.report.locadorNome} .xlsx`;
  ...
  async exportToPDF() {
    try {
        ...
        // Cabeçalho Principal
        doc.text(`Planilha de Base de Faturamento - ${this.selectedYear}`, 140, 15, null, null, 'center');
        ...
        this.report.locatarios.forEach(locatario => {
            ...
            currentY += 10;
            doc.text(`Endereço do imóvel: ${this.report.locadorEnderecoImovel} - Município: ${this.report.locadorMunicipioImovel} - UF: ${this.report.locadorUFImovel} - CEP: ${this.report.locadorCEPImovel}`,14,currentY);
            currentY += 10;
        ...
        // Salva o PDF
        doc.save(`DIMOB_${this.report.locadorNome} .pdf`);
